...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        scores.entry(team_1_name).and_modify(|sc| 
                                             { sc.goals_scored += team_1_score;
                                               sc.goals_conceded += team_2_score;
                                             }
                                             ).or_insert(Team { goals_scored: team_1_score, goals_conceded: team_2_score});
        scores.entry(team_2_name).and_modify(|sc| 
                                             { sc.goals_scored += team_2_score;
                                               sc.goals_conceded += team_1_score;
                                             }
